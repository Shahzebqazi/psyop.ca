  ...
  describe "Web API Tests" $ do
    with (return app) $ do
      describe "GET /" $ do
        ...
        it "renders the PSYOP title on home page" $ do
          response <- get "/"
          liftIO $ do
            simpleStatus response `shouldBe` status200
            simpleBody response `shouldSatisfy` 
              \body -> bodyContains "PSYOP" body &&
                       bodyContains "Electronic Music" body
          ...
          liftIO $ do
            ...
            simpleBody response `shouldSatisfy` 
              \body -> bodyContains "Moonlight Paradox" body &&
                       bodyContains "Great in Vain" body
          ...
          liftIO $ do
            ...
            simpleBody response `shouldSatisfy` 
              \body -> bodyContains "Home" body &&
                       bodyContains "Music" body &&
                       bodyContains "Links" body &&
                       bodyContains "Upcoming Shows" body
  ...
  describe "Image Sequence Model Tests" $ do
    ...
    it "nextImage advances sequence correctly" $ do
      let Just size = mkSequenceSize 5
          imgSeq = mkImageSequence size
          nextSeq = nextImage imgSeq
      currentIndex imgSeq `shouldBe` ImageIndex 0
    ...
    it "getCurrentImage returns correct index" $ do
      let Just size = mkSequenceSize 7
          imgSeq = mkImageSequence size
          currentIdx = getCurrentImage imgSeq
